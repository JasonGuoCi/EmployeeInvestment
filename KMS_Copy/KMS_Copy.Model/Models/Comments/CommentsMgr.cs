...
namespace KMS_Copy.Model.Models.Comments
{
    public class CommentsMgr
    {
        ...
        public List<CommentsEntity> GetCommentsByGUID(PagingEntity obj)
        {
            ...
            {
                return db.KMSComments.Where(p => p.ItemId == obj.Id).OrderBy(p => p.Created).Skip(obj.LimitRows * (obj.TakeCount - 1)).Take(obj.LimitRows).Select(p => new CommentsEntity(p)).ToList();
            }
        }
        /// <summary>
        /// 获取评论总数
        ...
        public int Delete(PagingEntity obj)
        {
            ...
            SettingsMgr mgr = new SettingsMgr();
            SettingsEntity setting = mgr.GetSystemSetting(obj.weburl, "DBConnction");
            using (CommentsLinqDataContext db = new CommentsLinqDataContext(setting.DefaultValue))
            {
