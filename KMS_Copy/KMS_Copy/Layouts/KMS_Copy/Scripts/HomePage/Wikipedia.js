var commentEntity = function (data, parent) {
    ...
    entity.RemoveComment = function () {
        ...
        if (r == true) {
            if (entity.Id()) {
                var method = "/_vti_bin/KMSServices/Global.svc/KMS/Global/Comments/Delete";
    ...
    entity.Payback = function () {
        if (entity.Comment().indexOf('<body>') == -1) {
            tinymce.activeEditor.setContent('<div class="focuswin-qutoes"><div>@' + entity.DisplayName() + '</div><div>' + entity.Comment() + '</div></div><p></p>');
        }
        else {
            tinymce.activeEditor.setContent('<div class="focuswin-quotes"><div>@' + entity.DisplayName() + '</div></div>' + entity.Comment().substring(entity.Comment().indexOf('<body>') + 6, entity.Comment().indexOf('</body>')) + '</div></div><p></p>');
        }
    }

    if (data) {
        entity.id = ko.observable(data.Id);
        entity.Comment = ko.observable(data.Comments);
        entity.DisplayName = ko.observable(data.DisplayName);
        entity.Create(SP2013.O365.Utility.formatFloatDate(data.Created));
    }
...
var wikiMgr = function () {
    ...
    self.Load = function () {
        var detailmethod = "/_vti_bin/KMSServices/Global.svc/KMS/Wikipedia/Content";
        var opts = {
            weburl: SP2013.O365.currentWebAbsoluteUrl(),
            LimitRows: 5000,
            TotalCount: 0,
    ...
    self.validateUser = function () {
        var method = "/_vti_bin/KMSServices/Global.svc/KMS/Global/Admin/Validation";
    ...
    self.InitWikiTree = function (opts) {
        ...
        var method = "/_vti_bin/KMSServices/Global.svc/KMS/Wikipedia/tree";
        var detailmethod = "/_vti_bin/KMSServices/Global.svc/KMS/Wikipedia/Content";
        ...
        SP2013.Utility.ReturnJson(SP2013.O365.postJSON(method, opts)).then(function (d) {
            ...
            $("#wikitree").jstree({
                ...
                if (eventNodeName == "A" || eventNodeName == "FONT") {
            ...
            if (d) {
                $("#left-nav").jstree({
                    'core': {
                        ...
                        {
                            "text": d.item1,//文本
                            "li_attr": { 'url': 'Wikipedia.aspx' },//生成li节点的属性
                            'state': { 'selected': true }  //是否选择节点
                        },
                        {
                            "text": d.item2,
                    ...
                    var eventNodeName = event.target.nodeName;　//获取事件触发元素标签name（li,p...）
                    if (eventNodeName == "A") {
    ...
    self.SearchTree = function () {
        ...
        if (self.keywords()) {
            self.IsSearch(true);
            var method = "/_vti_bin/KMSServices/Global.svc/KMS/Wikipedia/tree/Search";
            var detailmethod = "/_vti_bin/KMSServices/Global.svc/KMS/Wikipedia/Content";
    ...
    self.InitCommentsCount = function () {
        self.TotalCount(0);
        var method = "/_vti_bin/KMSServices/Global.svc/KMS/Global/Comments/ItemsCount";
        SP2013.Utility.ReturnJson(SP2013.O365.postJSON(method, {
            weburl: SP2013.O365.currentWebAbsoluteUrl(),
    ...
    self.InitComments = function () {
        ...
        var method = "/_vti_bin/KMSServices/Global.svc/KMS/Global/Comments/Items";
        SP2013.Utility.ReturnJson(SP2013.O365.postJSON(method, {
            weburl: SP2013.O365.currentWebAbsoluteUrl(),
            Id: self.CurrentwikiId(),
    ...
    self.submitcomments = function () {
        ...
        if (comments) {
            var method = "/_vti_bin/KMSServices/Global.svc/KMS/Global/Comments/Insert";
            SP2013.Utility.ReturnJson(SP2013.O365.postJSON(method, {
                weburl: SP2013.O365.currentWebAbsoluteUrl(),
                Id: self.CurrentwikiId(),
                Search: comments
    ...
    /*页面切换显示*/
    self.initPagination = function myfunction() {
